...
class GasEMEAProcessor:
    ...
    def _assign_tenors(self):
        """Assign, per trade, BOTH the recomputed periodicity bucket and the tenor.

        Writes two columns to self.df_trades:
        ...
        def _row(r):
            ...
            # BOM is a tenor-level refinement of an 'Other' contract (it needs the
            # reference date), so surface it in the periodicity column too.
            label = "BOM" if tenor == "BOM" else bucket.capitalize()
            return pd.Series({"periodicity_2": label, "tenor": tenor})

        self.df_trades[["periodicity_2", "tenor"]] = self.df_trades.apply(_row, axis=1)
    ...
    def _compute_vwap(self):
        ...
        # periodicity_2 is functionally determined by tenor, so adding it to the
        # grouping does not split any group - it just carries the label through.
        grouped = (
            df.groupby(["reference_date", "CLASSIFICATION_1", "periodicity_2", "tenor"], observed=True)
            .agg(
                pv_sum=("_pv", "sum"),
        ...
        self.df_vwap = result[
            ["reference_date", "weekday", "product", "periodicity_2", "tenor", "vwap",
    ...
    def _compute_tenor(self, ref_date: pd.Timestamp, contract_start: pd.Timestamp,
        ...
        # --- Non-standard contract -> BOM (balance of current month) or Custom
        # e.g. a 9-day mid-week strip, a partial month, a broker custom range.
        # Never forced into M+n, so it cannot contaminate a standard-bucket VWAP.
        else:
            return self._current_month_tenor(ref_date, contract_start, contract_end)
